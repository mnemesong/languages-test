...
function Yandex2Func_updateLast(a,fn) {
...
function Yandex2Func_run(str1) {
	var symbols = str1.split("");
	var foldF = function(s,acc) {
		if(acc.length == 0) {
			return acc.concat([{ el : s, c : 1}]);
		} else if(acc[acc.length - 1].el == s) {
			return Yandex2Func_updateLast(acc,function(el) {
				return { el : el.el, c : el.c + 1};
			});
		} else {
			return acc.concat([{ el : s, c : 1}]);
		}
...
function Yandex2Func_main() {
	var str1 = "AAABBBCCXYZDDDDAAAABB";
	var v = Yandex2Func_run(str1);
