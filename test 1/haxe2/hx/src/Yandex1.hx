...
class Yandex1 {
	...
	public static function arrayIntersect(
		arr1 : Array<Int>,
		arr2 : Array<Int>
	) : Array<Int> {
		var uniq = arr1.fold(
		...
		var counters : Array<Array<Int>> = uniq.map( s -> {
			var arrIncludesCnt = ( arr : Array<Int> ) -> arr.filter( el ->
				el == s ).length;
			var cnt = Std.int(
				Math.min( arrIncludesCnt( arr1 ), arrIncludesCnt( arr2 ) )
			);
			return [for ( i in 0...cnt ) s];
		} );
		return counters.fold( ( el : Array<Int>, acc : Array<Int> ) ->
			acc.concat( el ), [] );
	}
